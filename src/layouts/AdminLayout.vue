<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-xl transform transition-transform duration-300 ease-in-out"
      ...
      <!-- Sidebar Header -->
      <div class="flex items-center justify-between h-20 px-6 bg-gradient-to-r from-blue-600 to-blue-700">
        ...
        <button @click="toggleSidebar" class="text-white hover:text-blue-200 lg:hidden">
    ...
    <div v-if="sidebarOpen && windowWidth < 1024" class="fixed inset-0 z-40 bg-gray-600 bg-opacity-75" @click="closeSidebar"></div>

    <!-- Main Content -->
    <div class="transition-all duration-300 ease-in-out"
         :class="{ 'lg:pl-64': sidebarOpen || windowWidth < 1024, 'lg:pl-0': !sidebarOpen && windowWidth >= 1024 }">
      <!-- Top Bar -->
        ...
        <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
          <div class="flex items-center">
            <button 
              ...
              <component :is="sidebarOpen ? ChevronLeftIcon : Bars3Icon" class="h-6 w-6" />
            </button>
...
const navigation = [
  ...
  { name: 'Menu', label: 'จัดการเมนู', href: '/menu', icon: RectangleStackIcon },
  { name: 'Categories', label: 'หมวดหมู่', href: '/categories', icon: Squares2X2Icon },
  { name: 'Tables', label: 'จัดการโต๊ะ', href: '/tables', icon: TableCellsIcon },
  { name: 'Orders', label: 'คำสั่งซื้อ', href: '/orders', icon: ClipboardDocumentListIcon },
  { name: 'Reservations', label: 'การจอง', href: '/reservations', icon: CalendarDaysIcon },
